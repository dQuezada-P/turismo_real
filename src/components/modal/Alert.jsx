export const Alert = ({ alert }) => {
  ...
  return (
    <>
      <div className="mx-6 md:mx-40 md:h-[calc(100vh-176px)]">
        <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
          <div className="relative w-auto my-6 mx-auto max-w-sm">
            <div
              className={
                alert
                  ? "bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 p-8 shadow-md"
                  : "bg-red-100 border-t-4 border-red-500 rounded-b text-red-900 p-8 shadow-md"
              }
              role="alert"
            >
              <div className="flex flex-row items-center">
                ...
                <div className="basis-3/5 w-4/5">
                  <p className="font-bold text-2xl capitalize">{alert ? 'Reserva Exitosa' : 'No Se pudo completar la reserva'}</p>
                  <p className="text-sm">{alert ? `Departamento: ${"#"}` : "Error al intentar comprar"}</p>
                </div>
              </div>
            </div>
          </div>
